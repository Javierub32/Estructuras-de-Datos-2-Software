...
public class BinaryHeap<T> implements Heap<T> {
  ...
  public static <T> BinaryHeap<T> copyOf(BinaryHeap<T> that) {
    BinaryHeap<T> heap = new BinaryHeap<>(that.comparator, that.size, (T[]) new Object[that.elements.length]);
    System.arraycopy(that.elements, 0, heap.elements, 0, that.size);
    return heap;
  }
  ...
  private boolean hasRightChild(int index) {
    return isNode(rightChild(index));
  }
  ...
  private void heapifyDown(int index) {
    while(!isLeaf(index)){
      ...
      if(lessThan(index, smallestChildIndex)) {
        break;
      }
  ...
  public void deleteMinimum() {
    if(isEmpty()) {
      throw new EmptyHeapException("deleteMinimum");
    }
