...
public class MaxiphobicHeap<T> implements Heap<T> {
  ...
  private static <T> Node<T> copyOf(Node<T> node) {
    for (Node<T> child : Arrays.asList(node.left, node.right)) {
      if (child != null) {
        copyOf(child);
      }
    }
    return new Node<>(node.element, node.weight, node.left, node.right);
  }
  ...
  private Node<T> merge(Node<T> node1, Node<T> node2) {
    ...
    int w1 = weight(node2);
    int w2 = weight(node1.right);
    int w3 = weight(node1.left);
    Node<T> nodo1, nodo2, nodo3;
    ...
    Node<T> merged = merge(nodo1, nodo2);
