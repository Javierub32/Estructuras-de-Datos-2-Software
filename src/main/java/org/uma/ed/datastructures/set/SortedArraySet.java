...
public class SortedArraySet<T> extends AbstractSortedSet<T> implements SortedSet<T> {
  ...
  public static <T> SortedArraySet<T> copyOf(SortedSet<T> that) {
    SortedArraySet<T> sortedArraySet = new SortedArraySet<>(that.comparator());
    for (T element : that) {
      sortedArraySet.insert(element);
    }
    return sortedArraySet;
  ...
  public static <T> SortedArraySet<T> union(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    while (e1 != null && e2 != null) {
      ...
      if (cmp < 0) { // Si e1 es menor que e2, añadimos e1 al conjunto unión
        union.append(e1);
        e1 = nextOrNull(it1);
      } else if (cmp > 0) { // Si e2 es menor que e1, añadimos e2 al conjunto unión
        union.append(e2);
        e2 = nextOrNull(it2);
      } else { // Si e1 y e2 son iguales, añadimos uno de ellos al conjunto unión
        union.append(e1);
        e1 = nextOrNull(it1);
        e2 = nextOrNull(it2);
      }
    ...
    while (e1 != null) { // Añadimos los elementos restantes de it1 al conjunto unión
      union.append(e1);
      e1 = nextOrNull(it1);
    }

    while (e2 != null) { // Añadimos los elementos restantes de it2 al conjunto unión
      union.append(e2);
      e2 = nextOrNull(it2);
    }
  ...
  public static <T> SortedArraySet<T> intersection(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    while (e1 != null && e2 != null) {
      int cmp = comparator.compare(e1, e2);
      if (cmp < 0) { // Si e1 es menor que e2, avanzamos en it1
        e1 = nextOrNull(it1);
      } else if (cmp > 0) { // Si e2 es menor que e1, avanzamos en it2
        e2 = nextOrNull(it2);
      } else { // Si e1 y e2 son iguales, añadimos uno de ellos al conjunto intersección
  ...
  public static <T> SortedArraySet<T> difference(SortedSet<T> sortedSet1, SortedSet<T> sortedSet2) {
    ...
    while (e1 != null && e2 != null) {
      int cmp = comparator.compare(e1, e2);
      if (cmp < 0) { // Si e1 es menor que e2, añadimos e1 al conjunto diferencia
        difference.append(e1);
        e1 = nextOrNull(it1);
      } else if (cmp > 0) { // Si e2 es menor que e1, avanzamos en it2
        e2 = nextOrNull(it2);
      } else { // Si e1 y e2 son iguales, avanzamos en ambos iteradores
        e1 = nextOrNull(it1);
        e2 = nextOrNull(it2);
      }
    }

    while (e1 != null) { // Añadimos los elementos restantes de it1 al conjunto diferencia
      difference.append(e1);
      e1 = nextOrNull(it1);
    }
