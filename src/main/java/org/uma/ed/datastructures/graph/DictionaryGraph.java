...
public class DictionaryGraph<V> implements Graph<V> {
  ...
  private void addDiEdge(V source, V destination) {
    if(!vertices.contains(source)) {
      throw new IllegalArgumentException("Source vertex " + source + " is not in graph");
    }
    if(!vertices.contains(destination)) {
      throw new IllegalArgumentException("Destination vertex " + destination + " is not in graph");
    }

    Set<V> destinations = diEdges.valueOf(source);
    if(destinations == null) {
      destinations = JDKHashSet.empty();
